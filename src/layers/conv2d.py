...
class Conv2D:
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, stride: int = 1, padding: int = 1, seed: int = 42):
        ...
        #random number generator (initialization always on CPU)
    ...
    def _im2col(self, X, H_out: int, W_out: int):
        xp = backend.xp
        ...
        X = xp.ascontiguousarray(X, dtype=np.float32)
        ...
        X_view = xp.lib.stride_tricks.as_strided(
            X,
            shape=view_shape,
            strides=new_strides,
        )
        ...
        return xp.ascontiguousarray(X_col, dtype=np.float32)
    ...
    def _col2im(self, dX_col, X_shape: tuple, H_out: int, W_out: int):
        xp = backend.xp
        ...
        dX = xp.zeros(X_shape, dtype=dX_col.dtype)
    ...
    def forward(self, X):
        ...
        if self.padding > 0:
            X_padded = xp.pad(X, ((0, 0), (0, 0), (self.padding, self.padding), (self.padding, self.padding)), mode='constant', constant_values=0)
        else:
            X_padded = X
        ...
        W_col = xp.ascontiguousarray(W_col, dtype=np.float32)
    ...
    def backward(self, dY):
        ...
        dY_col = xp.ascontiguousarray(dY_col, dtype=np.float32)
