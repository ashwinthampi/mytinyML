...
class BatchNorm2D:
    ...
    def forward(self, X):
        ...
        if self.training:
            ...
            #mean per channel: (C,)
            self._mean = xp.mean(X_reshaped, axis=(0, 2))

            #variance per channel: (C,)
            self._var = xp.mean((X_reshaped - self._mean[:, None]) ** 2, axis=(0, 2))

            #standard deviation: (C,)
            self._std = xp.sqrt(self._var + self.epsilon)
            ...
            self._std = xp.sqrt(self.running_var + self.epsilon)
    ...
    def backward(self, dY):
        ...
        sum_dX_norm = xp.sum(dX_norm_reshaped, axis=(0, 2), keepdims=True)  #(1, C, 1)
        sum_dX_norm_x_norm = xp.sum(dX_norm_reshaped * X_norm_reshaped, axis=(0, 2), keepdims=True)  #(1, C, 1)
