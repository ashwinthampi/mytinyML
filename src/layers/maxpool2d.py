...
class MaxPool2D:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        if s == p:
            ...
            #reshape to (N, C, H_out, pool_size, W_out, pool_size)
            X_reshaped = X.reshape(N, C, H_out, p, W_out, p)
            #transpose to (N, C, H_out, W_out, pool_size, pool_size) then flatten pool dims
            X_blocks = X_reshaped.transpose(0, 1, 2, 4, 3, 5).reshape(N, C, H_out, W_out, p * p)

            #max and argmax over the last axis (pool window)
            self._argmax = np.argmax(X_blocks, axis=4).astype(np.int32)
            Y = np.max(X_blocks, axis=4)
        else:
            ...
            X_windows = np.lib.stride_tricks.as_strided(X, shape=view_shape, strides=view_strides)
    ...
    def backward(self, dY: np.ndarray) -> np.ndarray:
        ...
        h_out_grid = np.arange(H_out)[None, None, :, None]  #(1, 1, H_out, 1)
        w_out_grid = np.arange(W_out)[None, None, None, :]  #(1, 1, 1, W_out)
        ...
        n_idx = np.broadcast_to(n_idx, (N, C, H_out, W_out))
        c_idx = np.broadcast_to(c_idx, (N, C, H_out, W_out))

        #scatter-add gradients to argmax positions (handles potential overlaps for overlapping pooling)
        np.add.at(dX, (n_idx, c_idx, h_in_positions, w_in_positions), dY)
